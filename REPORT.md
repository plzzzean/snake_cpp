...
본 프로젝트는 C++과 `ncurses`를 사용해 터미널에서 실행되는 Snake Game을 구현하는 것을 목표로 한다. 이 수정본의 담당 범위는 맵 로드, Snake 이동, 충돌 처리, 스테이지 맵 선택, 화면 출력, 빌드 및 테스트 구조이다.
...
- `Game` 클래스가 입력 처리, Tick 기반 이동, 게임 오버 상태, 스테이지별 맵 선택을 관리한다.
- `Renderer` 클래스가 `ncurses` 초기화, 종료, 맵과 Snake 출력을 담당한다.
- `tests/test_logic.cpp`에서 맵 로드, 스테이지 맵 검증, Snake 이동 규칙을 assert 기반으로 테스트한다.
...
| 게임 흐름 | `Game` 클래스, 입력 처리, 게임 오버 상태 |
| 화면 출력 | `Renderer` 클래스, ncurses 기반 맵/Snake 표시 |
| 품질 관리 | Makefile, 테스트 코드, 파일별 역할 주석, `const` 점검 |

Food, Poison, Gate의 실제 동작 구현은 팀 내 다른 담당 영역이므로 이 수정본에서는 새 기능으로 추가하지 않았다. 다만 팀 통합을 위해 `CellType`에는 해당 셀 값을 유지했다.
...
- `Map`, `Snake`, `Game`, `Renderer`가 각각 어떤 역할인지 코드 파일을 열어 설명한다.
- 본인 담당 범위와 food/poison/gate 담당 범위가 분리되어 있음을 보고서 기준으로 설명한다.

## 7. 어려웠던 점과 개선점

맵 파일을 여러 크기의 스테이지로 관리하면서도 Snake 시작 위치와 충돌 판정이 안정적으로 동작하도록 검증하는 부분이 중요했다. 이를 위해 모든 스테이지 맵의 크기, 외곽 벽, 중앙 시작 구간을 테스트로 확인했다.

추후 팀 통합 단계에서는 별도 담당자가 구현한 Food, Poison, Gate 클래스를 `Game` 루프와 `Renderer` 출력에 연결하면 전체 과제 요구사항을 완성할 수 있다.
