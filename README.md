...
- `src/Map.*`: 맵 파일 로드, 기본 맵 생성, 벽 충돌 판정을 담당합니다.
...
다른 스테이지를 실행하려면 `src/Game.hpp`의 `stageLevel` 기본값을 바꾸거나, 이후 main에서 `GameConfig`를 넘기는 방식으로 확장하면 됩니다.
...
이후 단계에서 Item과 Gate를 추가할 때는 `CellType`, `Map`, `Snake`, `Game`의 상호작용을 확장하면 됩니다.
