...
현재 코드의 실제 담당 범위는 `Map`, `Snake`, `Game`, `Renderer`, `Food`, `Poison`, `Gate`입니다.
Stage3 Item 기능과 Stage4 Gate 기능이 구현되어 있으며, Score/Mission 기능은 이후 단계에서 연결할 영역입니다.
...
- 전체 Item은 최대 3개이며, 현재 구현은 Growth Item 2개 + Poison Item 1개 구조입니다.
...
- Gate 통과 후 Gate는 제거되며, 10초 후 새 Gate 쌍이 다시 출현합니다.
...
`Growth Item`과 `Poison Item`은 게임 중 `Food`, `Poison` 클래스가 빈 칸에 배치하며, `Gate` 셀 값은 다음 단계 통합을 위해 유지합니다.
...
4. `Food`와 `Poison`이 Growth Item 2개, Poison Item 1개를 빈 칸에 배치합니다.
5. 매 프레임 키 입력을 확인하고, Tick 시간이 지나면 Snake를 이동시킵니다.
6. Snake가 Item을 획득하면 길이 증가/감소 효과를 적용하고 해당 Item을 재생성합니다.
7. 5초가 지난 Item은 새 빈 위치에 재생성됩니다.
8. 게임 시작 후 10초가 지나면 `Gate::spawn()`이 Wall 위치 두 곳에 Gate 쌍을 생성합니다.
9. Snake 머리가 Gate 위치에 도달하면 출구 방향을 계산해 순간이동시키고 Gate를 제거합니다.
10. 충돌 또는 길이 3 미만 상태가 발생하면 게임 오버 상태로 전환하고, `q` 입력을 기다립니다.
11. 종료 시 `Renderer::shutdown()`이 터미널 상태를 복구합니다.
