...
- `src/Game.*`: 게임 루프, 입력 처리, Tick 기반 이동, 스테이지별 맵 선택, 게임 오버 상태, 아이템 초기 배치와 재생성 호출을 관리합니다.
...
현재 코드의 실제 담당 범위는 `Map`, `Snake`, `Game`, `Renderer`, `Food`, `Poison`입니다.
Stage3 Item 기능은 구현되어 있으며, Gate와 Score/Mission 기능은 이후 단계에서 연결할 영역입니다.
...
8. 충돌 또는 길이 3 미만 상태가 발생하면 게임 오버 상태로 전환하고, `q` 입력을 기다립니다.
9. 종료 시 `Renderer::shutdown()`이 터미널 상태를 복구합니다.
